...
const benefits = [
  { icon: GraduationCap, title: 'Опыт не нужен', text: 'Поможем разобраться во всем с нуля — от заявки до первого заказа.', tone: 'plain' },
  { icon: FileCheck2, title: 'Поможем с самозанятостью', text: 'Оформим и настроим все, чтобы вы работали полностью легально, без рисков.', tone: 'plain' },
  { icon: HeartPulse, title: 'Медкнижка бесплатно', text: 'Оформим без очередей и затрат — полное сопровождение.', tone: 'plain' },
  { icon: Trophy, title: 'Бонусы и реферальная программа', text: 'Дополнительный доход за приглашенных друзей и участие в турнирах.', tone: 'plain' },
  { icon: ShieldCheck, title: '10+ лет с иностранными гражданами', text: 'Знаем все юридические тонкости и законы РФ — не подведем с документами.', tone: 'sage' },
  { icon: Headphones, title: 'Поддержка 24/7 на 7 языках', text: 'На связи в любое время суток на вашем родном языке.', tone: 'forest' },
];
...
export const Benefits = () => (
  <section id="benefits" className="home-section">
    <div className="container">
      ...
      <div className="home-benefit-grid">{benefits.map(({ icon: Icon, title, text, tone }, i) => <article key={title} className={`home-benefit-card home-benefit-card--${tone}`}><div className="home-benefit-top"><Icon size={29} strokeWidth={1.5} /><span>0{i + 1}</span></div><h3>{title}</h3><p>{text}</p></article>)}</div>
    </div>
  </section>
);
